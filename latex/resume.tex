...
% \makeheading{Curriculum Vitae\\ [0.3cm] TIEP HUU VU\quad~~~~~~\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad{\small Last update: December 17, 2015}}
\makeheading{CHEN Han \hfill {\small Last update: Augest 20, 2024}}
...
\texttt{Address}: 05-02, West Coast Residental Village, 127371, Singapore\\												
\end{tabular}
\end{minipage}
...
\href{https://www.nus.edu.sg/}{\textbf{National University of Singapore}}, Singapore. \hfill 2023--2025(Expected)
        \begin{innerlist}
                \item Master of Computing, Computer Science Specialization. \hfill GPA: 4.3/5.0
        \end{innerlist}
...
\item {\it\textbf{Research Assistant}} in \textbf{National University of Singapore}\\
Mentor: \href{https://www.comp.nus.edu.sg/~hebs/}{Prof. Bingsheng HE}\hfill May-Sept.  2024\\
\vspace{-0.2in}
\begin{innerlist}
        \item Research on efficient inference for large language models(LLMs).
        \item Deploy a Qwen-72B model inference with tensor parallel and 4-bit quantized KV cache and speculative decoding method on 4 NVIDIA A100 GPU system for partner corporation 4 Paradigm. 
        \item Design a new 2-bit KV Cache quantization for LLMs, which can reduce the memory usage by 4-8x with more than 90\% accuracy retention on complex tasks like GSM8K and Code Completion.
        \item On submission to ICLR 2025.
\end{innerlist}

\item {\it\textbf{Internship}} in \textbf{SG Digital Trust Lab, Singapore Research Center, 2012 Laboratory}\\
Mentor: Dr. Tao HUANG\hfill Jan.-May  2024\\
\vspace{-0.2in}
\begin{innerlist}
        \item Research on high-performance symmetric encryption algorithm and SIMD optimization with AES instructions.
        \item Optimize LOL-MINI-NMH algorithm with scroll array and XOR fusion feature of KUNPENG 920 processor to improve the performance from 7.1Gbps to 8.5Gbps.
        \item Realize a new stream cipher algorithm with 49Gbps performance on KUNPENG 920 processor with the same AES instructions involved(3:1) as SOTA method Rocca which run at 38Gbps on the same processor, and also achieve a 1.3-1.4x performance over Rocca on other ARM processors.
        \item On submission to FSE 2025.
\end{innerlist}
...
% \makeheading{Curriculum Vitae\\ [0.3cm] TIEP HUU VU\quad~~~~~~\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad{\small Last update: December 17, 2015}}
\makeheading{陈 涵 \hfill {\small 最近更新: 2024年8月20日}}
...
%\texttt{Gitee:}\href{https://www.gitee.com/Concyclics}{www.gitee.com/concyclics}\\
%\texttt{Kaggle:}\href{https://www.kaggle.com/concyclics}{www.kaggle.com/concyclics} \\
\texttt{地址}: 新加坡 West Coast Residental Village 05-02, 127371\\												
\end{tabular}
\end{minipage}
\begin{minipage}[r]{0.5\textwidth}
\vspace{0.1in}
...
\begin{innerlist}
        \item 计算机科学硕士, 计算机科学方向. \hfill GPA: 4.3/5.0
\end{innerlist}
...
\item {\it\textbf{科研助理}}: 新加坡国立大学 \hfill 2024/05--2024/09\\
        导师: \href{https://www.comp.nus.edu.sg/~hebs/}{何丙胜教授}
        \begin{innerlist}
                \item 研究大型语言模型(LLMs)的高效推理。
                \item 通过张量并行和4位量化KV缓存, 以及推理过程中的投机采样的方法, 在4 NVIDIA A100 GPU系统上为合作公司第四范式部署了Qwen-72B模型推理。
                \item 设计了一种新的2-bit KV缓存量化方法, 可以在复杂任务如GSM8K和代码补全中减少4-8倍的内存使用, 并保持90\%以上的准确性。
                \item 该项目计划投稿至ICLR 2025。
        \end{innerlist}

\item {\it\textbf{实习生}}: 华为2012实验室新加坡研究所数字信任实验室 \hfill 2024/01--2024/05\\
        导师：黄涛博士
        \begin{innerlist}
                \item 研究利用SIMD指令集实现的高性能的流式对称密码算法。
                \item 通过滚动数组优化和鲲鹏920处理器的异或指令融合特性, 将LOL-MINI-NMH算法的性能从 7.1Gbps提升到8.5Gbps。
                \item 实现了一种新的对称流密码算法, 与当前SOTA算法Rocca相比, 在使用相同比例AES指令(3:1) 的情况下, 在鲲鹏920处理器上达到了52Gbps的性能, Rocca算法在该处理器上性能为38Gbps，同时在其他型号ARM处理器上也有约30-40\%的性能提升。
                \item 该项目计划投稿至FSE 2025。
        \end{innerlist}
